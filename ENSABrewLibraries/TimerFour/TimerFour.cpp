...
void TimerFour::setPwmDuty(char pin, int duty)
{
  ...
  if(pin == 6) OCR4B = dutyCycle;
  if(pin == 7) OCR4C = dutyCycle;
}

#if defined(__AVR_ATmega1280__) || defined(__AVR_ATmega2560__)
// DIsable for user with MegaCore
void TimerFour::pwm(char pin, int duty, long microseconds)  // expects duty cycle to be 10 bit (1024)
{
  ...
  if(pin == 8) { DDRH |= _BV(PORTH3); TCCR4A |= _BV(COM4A1); }
  if(pin == 6) { DDRH |= _BV(PORTH4); TCCR4A |= _BV(COM4B1); }
  if(pin == 7) { DDRH |= _BV(PORTH5); TCCR4A |= _BV(COM4C1); }
  setPwmDuty(pin, duty);
  start();
}
