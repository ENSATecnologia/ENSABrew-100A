...
void globalInterruption(void ) 
{
  ...
  if(_auxFlag1000ms) 
  {
    ...
    if(wtdRiseSetPoint != 0)
    {
      ...
      if(wtdRiseSetPoint == 0)
      {
        // Incrementa 0.1 no SetPoint
        actualTempSetPoint += 0.1;
        // Recarrega
        wtdRiseSetPoint = TIME_SET_POINT_RISE;
      }
      else if(wtdRiseSetPoint >= 6)
      {
        // Sobe 0.1°C à cada 6 segundos
        if(wtdRiseSetPoint % 6 == 0)
        {
          // Incrementa 0.1 no SetPoint
          actualTempSetPoint += 0.1;  
        }
